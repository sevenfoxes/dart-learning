define(['dart_sdk'], function(dart_sdk) {
  ...
  check.checkListIndex = function(index, size, opts) {
    ...
    if (dart.notNull(index) < 0 || dart.notNull(index) >= dart.notNull(size)) {
      dart.throw(new core.RangeError.new(check._resolveMessage(message, dart.str`index ${index} not valid for list of size ${size}`)));
    }
    return index;
  };
